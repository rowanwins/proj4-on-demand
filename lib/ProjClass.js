...
import { isWkt, checkMercator, checkProjStr, isProjString } from './parseCode.js'
...
export default class Proj4 {
  ...
  async registerRequiredProjection (projectionId) {
    ...
    const projDetails = this._parseInputProjectToJson(projectionId)
  ...
  async addProjectionFromEpsgIo (epsgCode) {
    const response = await fetch(`https://epsg.io/${epsgCode}.wkt`)
    if (!response.ok) {
      return
    }
    const projId = await response.text()
    const projDetails = this._parseInputProjectToJson(projId)
  ...
  _parseInputProjectToJson (input) {
    if (isWkt(input)) {
      var out = wkt(input)
